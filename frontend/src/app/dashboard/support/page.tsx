...
export default function SupportPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'Open':
        return (<Badge className="bg-amber-50 text-amber-700 border-amber-200 border"><Clock className="w-3 h-3 mr-1" />Menunggu</Badge>);
      case 'Answered':
        return (<Badge className="bg-blue-50 text-blue-700 border-blue-200 border"><MessageSquare className="w-3 h-3 mr-1" />Dijawab</Badge>);
      case 'Closed':
        return (<Badge className="bg-slate-100 text-slate-600 border-slate-200 border"><CheckCircle className="w-3 h-3 mr-1" />Selesai</Badge>);
      default:
      ...
      <div className="flex items-center justify-between">
        ...
        <Dialog open={showNewTicket} onOpenChange={setShowNewTicket}>
          <DialogTrigger asChild>
            <Button data-testid="btn-tiket-baru" className="bg-teal-600 hover:bg-teal-700 rounded-full">
              <Plus className="w-4 h-4 mr-2" />Tiket Baru
            </Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader>
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                <Select value={newTicket.kategori} onValueChange={(v) => setNewTicket({...newTicket, kategori: v})}>
                  <SelectTrigger id="kategori" data-testid="select-kategori"><SelectValue placeholder="Pilih kategori" /></SelectTrigger>
                  <SelectContent>
            ...
            <DialogFooter>
              ...
              <Button onClick={handleSubmitTicket} disabled={submitting} data-testid="btn-kirim-tiket" className="bg-teal-600 hover:bg-teal-700">
                {submitting ? (<><Loader2 className="w-4 h-4 mr-2 animate-spin" />Mengirim...</>) : (<><Send className="w-4 h-4 mr-2" />Kirim Tiket</>)}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      ...
      <Card className="border-0 shadow-card bg-gradient-to-br from-teal-50 to-emerald-50">
        <CardContent className="p-6">
          <div className="flex items-start gap-4">
            <div className="p-3 rounded-xl bg-teal-100 text-teal-700"><HelpCircle className="w-6 h-6" /></div>
            <div>
              <h3 className="font-heading font-semibold text-slate-900">Butuh bantuan cepat?</h3>
              <p className="text-sm text-slate-600 mt-1">Cek FAQ kami atau kirim tiket untuk pertanyaan spesifik</p>
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="space-y-4">
              {tickets.map((ticket) => (
                <div key={ticket.ticket_id} className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className="text-xs">{ticket.kategori}</Badge>
                  ...
                  <p className="text-sm text-slate-600 mt-1 line-clamp-2">{ticket.pesan_user}</p>

                  {ticket.balasan_admin && (
                    <div className="mt-3 p-3 bg-white rounded-lg border border-teal-100">
                      <div className="flex items-center gap-2 mb-1">
                        <Badge className="bg-teal-50 text-teal-700 border-teal-200 border text-xs">Admin</Badge>
                        {ticket.updated_at && (<span className="text-xs text-slate-400">{formatDate(ticket.updated_at)}</span>)}
                      </div>
