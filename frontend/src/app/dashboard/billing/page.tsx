      ...
      <Card className="border-0 shadow-card bg-white overflow-hidden">
        ...
        <CardContent className="p-6">
          <div className="text-center mb-6">
            <p className="text-4xl font-heading font-bold text-slate-900">Rp 25.000</p>
            <p className="text-slate-500">/bulan</p>
          </div>

          <ul className="space-y-3 mb-6">
            {['Akses penuh ke semua fitur', 'Generate laporan unlimited', 'Simpan data pasien unlimited', 'Export PDF & CSV', 'Support prioritas'].map((feature, idx) => (
          ...
          {(user?.status_langganan === 'TRIAL' || user?.status_langganan === 'EXPIRED') && (
            <Button onClick={handlePayment} disabled={loading} data-testid="btn-bayar" className="w-full h-12 bg-orange-500 hover:bg-orange-600 rounded-xl font-heading font-semibold">
              {loading ? (<><Loader2 className="w-5 h-5 mr-2 animate-spin" />Memproses...</>) : (<><CreditCard className="w-5 h-5 mr-2" />Bayar Langganan</>)}
            </Button>
          )}
          ...
          {user?.status_langganan === 'ACTIVE' && (
            <div className="text-center p-4 bg-emerald-50 rounded-xl">
              ...
              <p className="text-sm text-emerald-700 font-medium">Langganan Anda aktif</p>
            </div>
          )}
        </CardContent>
      </Card>
